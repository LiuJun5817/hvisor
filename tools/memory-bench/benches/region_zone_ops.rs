...
use hvisor_memory_bench::{mapping, Fixture, Mapping, OpResult, PAGE_SIZE};
...
fn result_buffer(count: usize) -> Vec<MaybeUninit<OpResult>> {
    (0..count).map(|_| MaybeUninit::uninit()).collect()
}

fn check_results(results: &mut [MaybeUninit<OpResult>]) {
    for result in results {
        // SAFETY: every slot is written once by the immediately preceding
        // measured batch. Consume native results only after the timer stops.
        let result = unsafe { result.assume_init_read() };
        assert!(result.is_ok(), "region operation failed");
    }
}

fn populate(fixture: &mut Fixture, mappings: &[Mapping]) {
    ...
    group.bench_function(
        ...
        |b| {
            b.iter_custom(|rounds| {
                let mut results = result_buffer(workload.regions);
                let mut elapsed = Duration::ZERO;
                for _ in 0..rounds {
                    fixture.add_empty_zone().expect("zone setup failed");
                    let start = Instant::now();
                    for (region, result) in input.iter().zip(&mut results) {
                        result.write(fixture.insert(black_box(region)));
                    }
                    elapsed += start.elapsed();
                    check_results(black_box(&mut results));
        ...
        |b| {
            b.iter_custom(|rounds| {
                ...
                for _ in 0..rounds {
                    ...
                    let start = Instant::now();
                    for (region, result) in input.iter().zip(&mut results) {
                        result.write(fixture.remove(black_box(region)));
                    }
                    elapsed += start.elapsed();
                    check_results(black_box(&mut results));
                    fixture.check_empty();
